...
export function Contact(initialTourId?: string): HTMLElement {
  ...
  const renderContent = () => {
    ...
    contact.innerHTML = `
    <div>
      <h2 class="text-4xl font-bold text-gray-800 mb-6 text-center">${t('contact.title')}</h2>
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (!hasErrors) {
      ...
      const templateParams = {
        ...
        tour_details: isReservation
          ? `${tourOpt.getAttribute('data-title')} - Fecha: ${dateInput?.value}, Pasajeros: ${passengersInput?.value}. ESTIMADO TOTAL: $${estPrc} USD`
          : 'Contacto General (Sin Reserva específica)',
      };
