...
export function Testimonials(): HTMLElement {
  ...
  const renderContent = () => {
					...
					<div class="testimonial-card bg-white rounded-2xl shadow-xl p-6 mx-4 min-h-[450px] flex flex-col border border-gray-100 hover:border-emerald-200 transition-all duration-300">
						...
						<div class="mt-auto pt-4 border-t border-gray-100">
							<div class="w-full">
								...
								<p class="text-emerald-600 text-sm font-medium mt-1 truncate">${testimonial.location}</p>
							</div>
						</div>
					</div>
				</div>
    ...
    testimonials.innerHTML = `
			<div>
				<h2 class="text-3xl font-bold text-gray-800 mb-4 text-center">${t('testimonials.title')}</h2>
  ...
  const setupCarousel = (total: number) => {
    const track = testimonials.querySelector('.testimonials-track') as HTMLElement;
    ...
    const indicators = testimonials.querySelectorAll('.indicator') as NodeListOf<HTMLElement>;
